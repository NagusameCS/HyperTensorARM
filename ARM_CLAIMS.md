...
| 2 | COG 10K interactions: 14 trajectories, metric saturates, Mann-Kendall p=0.015 | Pending (hours-long CPU run on Qwen2.5-1.5B — model now available) | ⏳ T2 |
...
| 11 | HyperRetro compression: fp16 2.33 tok/s → int4 FFN-only+AWQ 6.04 tok/s (2.38×), 2955→1242 MB | GRC rank-1024 compression of all 28 layers runs on ARM; output is a standard HF checkpoint that loads/runs (PPL 14.58 vs 12.94). **Full E2E VERIFIED on 0.5B and 1.5B: Q4_K_M GGUF → ffn_rank=256+int4 → exported GGUF runs on geodessical AND llama.cpp; logits match a PyTorch dense reconstruction; tokenizer bit-identical to llama.cpp. ARM throughput table (their 2.38× claim → ours 10-11.5×): 0.5B fp16 37.7 tok/s → Q8_0 108.7 (2.9×, PPL unchanged 10.98) / Q4_K_M 109.2 (2.9×, PPL 11.24 vs llama oracle 10.83); 1.5B fp16 13.6 tok/s → Q4_K_M 34.6 (2.5×, PPL 6.94→7.08 vs llama oracle 6.73). fp16/F32 GEMVs are SMP NEON row-split** (`benchmarks/arm/compress_throughput_arm.json`) | ✅ MET (pipeline + loadability + E2E verified vs oracle + throughput table) |
