...
| 11 | HyperRetro compression: fp16 2.33 tok/s → int4 FFN-only+AWQ 6.04 tok/s (2.38×), 2955→1242 MB | GRC rank-1024 compression of all 28 layers runs on ARM; output is a standard HF checkpoint that loads/runs (PPL 14.58 vs 12.94). **Full E2E VERIFIED: 0.5B Q4_K_M GGUF → ffn_rank=256+int4 → exported GGUF (fp16, 1208 MB) runs on geodessical at 10.8 tok/s with coherent output ("France's capital, and the seat of government in that country:"); WikiText PPL 10.98 vs llama.cpp oracle 10.66 on the same file (Δ+0.3 PPL quant noise); logits match a PyTorch dense reconstruction; tokenizer is bit-identical to llama.cpp (560/560 tokens)**. Throughput table needs the AWQ-calibrated variant | ✅ MET (pipeline + loadability + E2E verified vs oracle), ⏳ (tok/s table) |
