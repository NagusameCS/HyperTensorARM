...
def _vocab_from_tokens(val) -> int:
    """Vocab size from the tokenizer.ggml.tokens array (GGUF stores the token
    list, not an explicit vocab-size KV in many models)."""
    if isinstance(val, (list, tuple)):
        return len(val)
    if val is not None and hasattr(val, "__len__") and not isinstance(val, (str, bytes)):
        return len(val)
    return 32000
...
class GGUFAdapter(AbstractModel):
    ...
    def __init__(self, gguf_path: str | Path, config_dict=None):
        try:
        ...
        self._config = config_dict if config_dict is not None else self._extract_config()
    ...
    def _extract_config(self) -> dict:
        ...
        _tok_field = f.get("tokenizer.ggml.tokens")
        _vocab = 32000
        if _tok_field is not None:
            _tok_data = getattr(_tok_field, "data", None)
            if _tok_data is not None and hasattr(_tok_data, "__len__"):
                _vocab = len(_tok_data)
        cfg = {
            ...
            "vocab_size": int(_get(f"{arch}.vocab_size", _vocab)),
            "_gguf_path": str(self._path),
    ...
    def state_dict(self) -> dict:
        """Lazy-load all tensors into memory (dequantizing Q4/Q5/Q6/Q8)."""
        if self._state_dict_cache is not None:
            return self._state_dict_cache

        import numpy as _np
        from gguf import GGMLQuantizationType, dequantize as _dequant
        ...
        for name, info in self._tensor_info.items():
            try:
                ...
                if data is None:
                    continue
                qt = GGMLQuantizationType(info["tensor_obj"].tensor_type)
                if qt in (GGMLQuantizationType.F32, GGMLQuantizationType.F16):
                    arr = _np.array(data)
                else:
                    # gguf-python's raw .data has wrong dims for quantized
                    # tensors; dequantize() restores the true shape.
                    arr = _dequant(data, qt)
                if _HAS_TORCH:
                    sd[name] = torch.from_numpy(_np.ascontiguousarray(arr, dtype=_np.float32))
                else:
                    sd[name] = _np.ascontiguousarray(arr, dtype=_np.float32)
            except Exception:
                continue
...
def _load_gguf(model_id: str, device=None, dtype=None, **kwargs):
    """Registry factory: returns (path, HF-compatible config dict). The
    adapter is then built by load_model with the extracted config (metadata
    is parsed twice — cheap: metadata only, tensors stay lazy)."""
    ad = GGUFAdapter(model_id)
    return str(ad._path), ad._config
