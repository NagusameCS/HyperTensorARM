...
#else
/* ARM64: same 8-wide float vector type; clang lowers it to NEON pairs so the
 * AVX2-tuned scalar-vector code remains valid (dead on ARM via call guards,
 * but must compile). Helpers are defined below in the ARM section. */
typedef float v8f __attribute__((vector_size(32)));
#endif

/* ARM64 implementations of the v8f helper surface (NEON). */
#ifdef __aarch64__
...
static inline float v8f_reduce(v8f v) {
    float32x4_t lo = vld1q_f32((const float *)&v);
    float32x4_t hi = vld1q_f32((const float *)&v + 4);
    return vaddvq_f32(vaddq_f32(lo, hi));
}
static inline v8f v8f_max(v8f a, v8f b) {
    float32x4_t lo = vmaxq_f32(vld1q_f32((const float *)&a), vld1q_f32((const float *)&b));
    float32x4_t hi = vmaxq_f32(vld1q_f32((const float *)&a + 4), vld1q_f32((const float *)&b + 4));
    v8f r; vst1q_f32((float *)&r, lo); vst1q_f32((float *)&r + 4, hi); return r;
}
static inline float32x4_t arm_vexp_est(float32x4_t x) {
    const float32x4_t log2e = vdupq_n_f32(1.4426950408889634f);
    const float32x4_t c0 = vdupq_n_f32(1.0f);
    const float32x4_t c1 = vdupq_n_f32(0.6931472f);   /* ln 2 */
    const float32x4_t c2 = vdupq_n_f32(0.24022651f);  /* ln2^2 / 2 */
    const float32x4_t c3 = vdupq_n_f32(0.05550411f);  /* ln2^3 / 6 */
    float32x4_t t = vmulq_f32(x, log2e);
    float32x4_t n = vcvtq_f32_s32(vcvtq_s32_f32(t));          /* floor via trunc */
    float32x4_t f = vsubq_f32(t, n);
    float32x4_t poly = vaddq_f32(c0, vmulq_f32(f, vaddq_f32(c1,
        vmulq_f32(f, vaddq_f32(c2, vmulq_f32(f, c3))))));
    int32x4_t e = vaddq_s32(vcvtq_s32_f32(n), vdupq_n_s32(127));
    e = vshlq_n_s32(e, 23);
    return vmulq_f32(poly, vreinterpretq_f32_s32(e));
}
static inline v8f v8f_expf(v8f v) {
    float32x4_t lo = arm_vexp_est(vld1q_f32((const float *)&v));
    float32x4_t hi = arm_vexp_est(vld1q_f32((const float *)&v + 4));
    v8f r; vst1q_f32((float *)&r, lo); vst1q_f32((float *)&r + 4, hi); return r;
}
#endif
...
static int64_t llm_alloc_kv_floats; /* int64_t: layersseqkv_headshd can exceed 2^31 on 70B+ models */
...
/* SIMD int8→float: vpmovsxbd (sign-extend 8i8→8i32) + vcvtdq2ps
...
/*  Integer Q8Q8 dot product (avoids ALL int→float conversion)  */
...
static void q8_quantize_block(q8_input_t *out, const float *x)
{
    ...
    /* Cache sum of quantized values for Q4Q8 bias correction */
    {
        int32_t s = 0;
...
/* Uses vpmaddubsw (unsignedsigned→int16) + vpmaddwd (int16→int32).
...
/* Fused Q4_0Q8_0 integer GEMV: 4 rows at a time with shared Q8 input */
__attribute__((target("avx2,fma")))
...
/* Fused Q4_0Q8_0 integer GEMV for a range of rows (SMP workers) */
__attribute__((target("avx2,fma")))
 ...
 * Uses VPMOVZXBD (8-byte load → 8int32) + AND + SUB + VCVTDQ2PS (1 cycle each). */
__attribute__((target("avx2,fma")))
                     ...
                     int out_dim, int in_dim, ggml_type_t type)
{
    ...
    /* Q4Q8 integer GEMV: pre-quantize input to Q8, then use integer dot */
    ...
    if (cpu_features.avx2_usable) {
        ...
        if (ncpu > 1 && out_dim >= 64 &&
            ...
            /* Q4Q8 integer path pending correctness fix — use float path */
    ...
    case GGML_TYPE_Q5_0: {
        ...
        for (int b = 0; b < nb; b++) {
            ...
            for (int j = 0; j < 16; j++) {
                int q_lo = (int)((qs[j] & 0xF) | (((qh >> j) & 1u) << 4)) - 16;
                int q_hi = (int)((qs[j] >> 4)  | (((qh >> (j + 16)) & 1u) << 4)) - 16;
                out[b * 32 + j]      = d * (float)q_lo;
                out[b * 32 + j + 16] = d * (float)q_hi;
            }
        }
        break;
    }
...
/* NeoX (half-rotation) RoPE mode: qwen2-family GGUFs rotate (v[i], v[i+h/2]).
 * Verified numerically against llama.cpp on Qwen2.5-0.5B (error < 1e-4). */
static int   llm_rope_neox_mode = 0;
                     ...
                     const float *factors)
{
     ...
     * Skipped in NeoX mode (JIT kernels rotate adjacent pairs). */
    if (factors == NULL && !llm_rope_neox_mode && jit_fwd_rope_hd && head_dim == llm_rope_hdim_cached) {
        jit_fwd_rope_hd(vec, pos, head_dim, llm_rope_freqs_buf);
        return;
    }
    /* Partial RoPE cache (Gemma4: rope_dim != head_dim) */
    if (factors == NULL && !llm_rope_neox_mode && head_dim == llm_rope_hdim_cached) {
    ...
    if (llm_rope_neox_mode) {
        /* NeoX: rotate (v[i], v[i + head_dim/2]) for i in [0, head_dim/2). */
        const int half = head_dim / 2;
        for (int i = 0; i < half; i++) {
            float theta = (float)pos * llm_rope_freqs_buf[i];
        ...
        for (int L = 0; L < m->n_layers; L++) {
            ...
            if (has_own_kv) {
                ...
                if (_cq_ && _cq_->d_Vt) {
                    ...
                    be->compute.rmsnorm(gpu_ctx.d_xn, gpu_ctx.d_x, gl->d_attn_norm,
                                        dim, gpu_ctx.rms_eps);
                    /* Project: d_x_sub[k] = Pt[kn] @ d_xn[n] (F16 Pt → 2 less bandwidth) */
            ...
            /* Priority: SVD compressed O (slot 4) > GP manifold > raw GPU weight */
            {
                ...
                if (_co_ && _co_->d_Vt) {
                    ...
                    lq_dim == _gp_n_o) {
                    /* x_sub_o[k] = Pt[kn] @ d_attn[n] (F16 Pt → 2 less bandwidth) */
...
#define GPU_FFN_DOWN_GEMV(_dst, _src, _dim, _lff) do { \
    ...
    if (_md7_ && _md7_->d_W_proj && _d_Qff_ && _k_ff_dn_ > 0) { \
        /* FFN down GP: x_sub[k_ff] = Qff[k_fflff] @ ffn_g[lff], then \
         * ffn_d[dim] = W_proj_down[dimk_ff] @ x_sub[k_ff]. \
...
#undef GPU_COMPRESSED_GEMV
    ...
    for (int L = 0; L < m->n_layers; L++) {
         ...
         * compute x_sub = P^T @ xn once, then use mk GEMVs for Q/K/V. */
...
#ifdef HT_DEBUG_FWD
        if (L == 0 && pos < 2 && getenv("HT_DEBUG_FWD")) {
            kprintf("[DBG] pos=%d embed=%.4f %.4f %.4f xn=%.4f %.4f %.4f q0=%.4f %.4f %.4f %.4f k0=%.4f %.4f %.4f %.4f v0=%.4f %.4f\n",
                pos, llm_x[0], llm_x[1], llm_x[2],
                llm_xn[0], llm_xn[1], llm_xn[2],
                llm_q[0], llm_q[1], llm_q[2], llm_q[3],
                llm_k_buf[0], llm_k_buf[1], llm_k_buf[2], llm_k_buf[3],
                llm_v_buf[0], llm_v_buf[1]);
            kprintf("[DBG] rope_base=%.6f rope_dim=%d freqs0..3=%.6f %.6f %.6f %.6f\n",
                m->rope_base, m->rope_dim,
                llm_rope_freqs_buf[0], llm_rope_freqs_buf[1],
                llm_rope_freqs_buf[2], llm_rope_freqs_buf[3]);
        }
#endif
        int kv_src_layer = has_own_kv ? L : layer->kv_reuse_layer;
        if (has_own_kv) {
...
static int llm_map_tensors(llm_model_t *m, gguf_ctx_t *ctx)
{
    ...
    for (int L = 0; L < m->n_layers; L++) {
        ...
        /* Fused gate+up tensor (Phi-3: ffn_up.weight has 2ff_dim rows) */
...
static int llm_init_parsed_model(llm_model_t *m)
{
    ...
    /* NeoX-style (half-rotation) RoPE: the qwen2 family GGUFs use it, matching
     * llama.cpp's per-arch rope default (verified numerically against the
     * official Qwen2.5-0.5B GGUF: half-rotation matches to <1e-4). */
    m->rope_neox = (kstrncmp(m->arch, "qwen2", 5) == 0) ? 1 : 0;
    llm_rope_neox_mode = m->rope_neox;
    kprintf("[LLM] RoPE mode: %s\n", m->rope_neox ? "neox (half-rotation)" : "adjacent-pairs");
    ...
    /* ARM64: allow explicit backend override for A/B testing (HT_BACKEND=cpu|arm) */
#ifdef __aarch64__
    {
        const char *hb = getenv("HT_BACKEND");
        if (hb && hb[0] == 'c' && backend_get_by_id(BACKEND_CPU))
...
void llm_gpu_upload_compressed_weights(void)
{
    ...
    for (int L = 0; L < m->n_layers; L++) {
         ...
         * exhaustion that would otherwise limit uploads to ~7 layers. */
        {
             ...
             * For 70B this recovers ~38 MB per layer (Q+O @ IQ2_XS 81928192  2). */
     ...
     * When GP is active, Q/K/V weights are replaced by W_proj[mk] — up to 64 smaller.
                              ...
                              int *output_tokens, int max_output_tokens)
{
    ...
    if (!input_tokens || n_input_tokens <= 0) return -1;
    if (executor)
...
/* Debug utility: forward over text and print per-position top-5 logits.
 * Used to diff the ARM runtime against llama.cpp reference output. */
void llm_debug_dump_topk(const char *text, int max_positions)
{
    if (!llm_is_loaded() || !text || max_positions <= 0) return;
    if (__sync_lock_test_and_set(&llm_inference_active, 1)) return;

    llm_model_t *m = &llm_model;
    int *tokens = (int *)kmalloc((size_t)(max_positions + 2) * sizeof(int));
    if (!tokens) { __sync_lock_release(&llm_inference_active); return; }
    int n = llm_tokenize_text(text, tokens, max_positions + 2);
    if (n >= 2) {
        m->cache_len = 0;
        llm_rep_reset();
        float *logits = (float *)kmalloc((size_t)m->vocab_size * sizeof(float));
        if (logits) {
            kprintf("[TOP5] n_tokens=%d tokens:", n);
            for (int i = 0; i < n && i < 12; i++) kprintf(" %d", tokens[i]);
            kprintf("\n");
            for (int i = 0; i < n - 1 && i < max_positions; i++) {
                llm_forward_token(m, logits, tokens[i], i);
                int ids[5] = {-1,-1,-1,-1,-1};
                ...
                for (int v = 0; v < m->vocab_size; v++) {
                    ...
                    for (int k = 0; k < 5; k++) {
                        if (x > vals[k]) {
                            for (int q = 4; q > k; q--) { ids[q] = ids[q-1]; vals[q] = vals[q-1]; }
                            ids[k] = v; vals[k] = x;
                            break;
                        }
                    }
                }
                kprintf("[TOP5] pos=%d next=%d top:", i, tokens[i+1]);
                for (int k = 0; k < 5; k++)
                    kprintf(" %d(%.2f)", ids[k], (double)vals[k]);
                kprintf("\n");
            }
            kfree(logits);
        }
...
/* Debug utility: run one forward token and dump the residual stream x after
 * each layer (first 4 elements). Used to diff against a Python reference. */
void llm_debug_trace_layers(const char *text, int pos)
{
    if (!llm_is_loaded() || !text || pos < 0) return;
    if (__sync_lock_test_and_set(&llm_inference_active, 1)) return;

    llm_model_t *m = &llm_model;
    int *tokens = (int *)kmalloc((size_t)(pos + 2) * sizeof(int));
    if (!tokens) { __sync_lock_release(&llm_inference_active); return; }
    int n = llm_tokenize_text(text, tokens, pos + 2);
    if (n >= pos + 1) {
        m->cache_len = 0;
        llm_rep_reset();
        float *logits = (float *)kmalloc((size_t)m->vocab_size * sizeof(float));
        if (logits) {
            for (int i = 0; i < pos; i++)
                llm_forward_token(m, logits, tokens[i], i);
            llm_forward_token(m, logits, tokens[pos], pos);
            /* The final layer output x lives in llm_x; print first 8 elems. */
            kprintf("[TRACE] pos=%d x:", pos);
            for (int i = 0; i < 8; i++) kprintf(" %.6f", (double)llm_x[i]);
            kprintf("\n");
            kfree(logits);
        }
    }
...
void llm_debug_forward_token(int token_id, int pos, int reset_first)
{
    ...
    if (logits) {
        ...
        float vals[6] = {-1e30f,-1e30f,-1e30f,-1e30f,-1e30f,-1e30f};
        for (int v = 0; v < m->vocab_size; v++) {
            for (int k = 0; k < 6; k++) {
                if (logits[v] > vals[k]) {
                    for (int q = 5; q > k; q--) { ids[q] = ids[q-1]; vals[q] = vals[q-1]; }
                    ids[k] = v; vals[k] = logits[v];
                    break;
                }
            }
        }
        kprintf("[FWD] tok=%d pos=%d top:", token_id, pos);
        for (int k = 0; k < 6; k++)
            kprintf(" %d(%.3f)", ids[k], (double)vals[k]);
        kprintf("\n");
        kfree(logits);
    }
    __sync_lock_release(&llm_inference_active);
