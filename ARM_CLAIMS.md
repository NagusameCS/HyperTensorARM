...
| 9 | GRC attention compression: 106% throughput at k=1024 (L2 cache residency) | **GRC method fully ported and verified on ARM**: shared-basis Q/K/V compression + sink-aware restore in the GGUF pipeline (in-memory + streaming, `--attn-rank`, `--sink`). 1.5B at k=1024 (2/3 of d=1536) stays sane: PPL 6.40 ours / 6.14 llama.cpp (streaming) vs int4-only 6.94/6.73 — matches the original's method-level claim (their 14.58 vs 12.94 on their eval set). The standalone 106% L2-residency throughput number is NVIDIA-hardware-specific | OK MET (method), PENDING T3 (106% L2 hardware number) |
...
| 11 | HyperRetro compression: fp16 2.33 tok/s → int4 FFN-only+AWQ 6.04 tok/s (2.38×), 2955→1242 MB | GRC rank-1024 compression of all 28 layers runs on ARM; output is a standard HF checkpoint that loads/runs (PPL 14.58 vs 12.94). **Full E2E VERIFIED on 0.5B and 1.5B against llama.cpp + PyTorch oracles; tokenizer bit-identical. 0.5B configs (PPL ours/llama): int4-only 10.98/10.66; ffn_rank=1024+int4 in-memory 14.89/14.20; ffn_rank=1024+int4 streaming 11.66/11.09 (861 MB, decode 46.4 tok/s); ffn_rank=256+int4 breaks (2M, oracle-consistent). 1.5B: int4-only 6.94/6.73; ffn_rank=1024 breaks; ffn_rank=2048+int4 streaming 7.73/7.35 (2729 MB, decode 15.9 tok/s). **GRC attention (ported): 1.5B attn_rank=1024+int4 streaming = 6.40/6.14 (1242.5 MB, 52 s); in-memory vanilla 7.46/7.20, sink_T=4 7.68/7.45; 0.5B attn_rank=600+int4 streaming sink_T=4 = 9.48/9.01 (523.4 MB).** ARM throughput table (their 2.38× → ours 10×): fp16 37.7 tok/s → Q8_0 108.7 / Q4_K_M 109.2 (0.5B). fp16/F32 GEMVs SMP NEON row-split. Streaming compressor: peak memory = one tensor, randomized SVD above 64M elements** (`benchmarks/arm/compress_throughput_arm.json`, `docs/E2E_PIPELINE.md`) | OK MET (pipeline + loadability + E2E verified vs oracle + throughput table + streaming) |
...
- **Python suites**: `pytest tests/` = 114/114 (incl. 44 ht-repro + 33
  hyperretro unit tests + GRC attention tests); commercial audit 33/33 (live MPS matmul).
