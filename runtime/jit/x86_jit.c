 ...
 *   2. For each block: load scale, process 32 int8float products
...
{
    /* Test: 44 identity  B = B  (same test as tensor_cpu_selftest) */
...
/* =============================================================================
 * AVX2 Integer SIMD Emitters (for Q4_0Q8_0 integer dot product)
 * =============================================================================*/
...
/* =============================================================================
 * AVX2 Q4_0Q8_0 Integer GEMV Kernel Compiler
 *
 ...
 *   2. vpmaddubsw(q4_unsigned, q8_signed) → 16int16
 *   3. vpmaddwd(result, all_ones) → 8int32
...
/* Helper: emit Q4Q8 block processing for one row.
...
static void emit_q4q8_row_block(jit_buf_t *b, int ymm_iacc, int ymm_facc)
{
    ...
    /* vpmaddubsw ymm5, ymm0, ymm2 → 16int16 (unsigned Q4  signed Q8) */
    jit_vpmaddubsw256(b, YMM5, YMM0, YMM2);

    /* vpmaddwd ymm5, ymm5, ymm3 (all-ones) → 8int32 */
    ...
    /* vpmaddwd ymm6, ymm6, ymm3 → 8int32 partial sums of Q8 bytes */
...
/* Compile a Q4_0Q8_0 integer GEMV kernel.
