...
typedef enum {
    ...
    BRIDGE_MODE_CAP_FFN   = 16, /* Capture FFN intermediate (SiLU(gate)⊙up) at EVERY layer (for FFN PCA) */
} bridge_mode_t;
...
typedef struct {
    ...
    /* Multi-layer capture (BRIDGE_MODE_CAP_ALL): residual stream [n_layers  dim] */
    ...
    /* FFN intermediate capture (BRIDGE_MODE_CAP_FFN): SiLU(gate)⊙up [n_layers  ff_dim] */
    float        *ffn_cap_bufs;     /* flat [n_layers  ff_dim] F32, caller-allocated */
    int           ffn_cap_n;        /* number of layers (0 = off) */
    int           ffn_cap_dim;      /* ff_dim per layer (may vary; use max ff_dim) */
    int          *ffn_cap_valid;    /* per-layer valid flags (caller-allocated int[n]) */
...
/* Set up FFN intermediate capture for BRIDGE_MODE_CAP_FFN.
 * Captures SiLU(gate)⊙up at every transformer layer in one forward pass.
 * bufs must be caller-allocated float[n_layers  ff_dim].
 * valid must be caller-allocated int[n_layers], zero-initialised.
 * ff_dim is the maximum ff_dim across all layers (use model->ff_dim). */
